...
public class SidebarSelectItem implements SidebarSelectLayer {
    ...
    public void paintSelection(Graphics g, DrawingArea bounds) {
        ...
        g.drawImage(iAssets.getMiscImage('+'),
                bounds.getXOffset() + GUISidebar.INVENTORY_OFFSET.x + (slot / GUISidebar.INVENTORY_ROWS * ImageAssets.SPRITE_DIMENSION_SM_PX),
                bounds.getYOffset() + GUISidebar.INVENTORY_OFFSET.y + (slot % GUISidebar.INVENTORY_ROWS * ImageAssets.SPRITE_DIMENSION_SM_PX),
                null);
    }
}
