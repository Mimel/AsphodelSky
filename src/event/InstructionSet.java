...
public class InstructionSet
{
     ...
     */
    private Map<Opcode, TriFunction<InstructionData, CompositeGrid, ResponseDetails>> instructionSet;

    private ResponseTable responses;

    /**
     * Private constructor used to prevent instantiation.
     */
    public InstructionSet(ResponseTable rt) {
        responses = rt;
        loadInstructionSet();
    }
     ...
     */
    String execute(Opcode opcode, InstructionData data, CompositeGrid gr) {
        if(instructionSet.containsKey(opcode)) {

            ResponseDetails rd = instructionSet.get(opcode).apply(data, gr);
            if(rd != null) {
                return responses.getResponse(opcode, rd);
            }
